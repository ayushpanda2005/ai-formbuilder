...
function FieldEdit({ defaultValue, onUpdate, deleteField }) {
  const [formLabel, setFormLabel] = useState(defaultValue.formLabel || "");
  const [placeholder, setPlaceholder] = useState(defaultValue.placeholder || "");

  // Update state when defaultValue changes
  useEffect(() => {
    setFormLabel(defaultValue.formLabel || "");
    ...
    <div>
      <Popover>
        ...
        <PopoverContent>
          ...
          <div className="mb-3">
            <label className="text-xs block mb-1">Form Label</label>
            <Input
              type="text"
              value={formLabel}
          ...
          <Button
            size="sm"
            className="mt-3"
            onClick={() => onUpdate({ formLabel, placeholder })}
          >
            Update
          </Button>
        </PopoverContent>
  ...
  <AlertDialogContent>
    ...
    <AlertDialogFooter>
      ...
      <AlertDialogAction onClick={() => deleteField(defaultValue.index)}>
  Continue
</AlertDialogAction>
